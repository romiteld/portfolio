...
export default function DemosPage() {
  ...
  useEffect(() => {
    if (demoCardsRef.current) {
      const ctx = gsap.context(() => {
        ...
        cards.forEach((card) => {
          gsap.to(card, { // Animate opacity based on scroll
  ...
  return (
    ...
    >
      ...
      <div ref={demoCardsRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 grid-auto-rows-auto">
        {demos.map((demo) => (
          <Link key={demo.slug} href={`/demos/${demo.slug}`} className="flex">
            <motion.div
              className="group bg-white dark:bg-neutral-800/50 backdrop-blur-sm rounded-xl border border-neutral-200/50 dark:border-neutral-700/50 p-6 shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col flex-grow h-full"
